...
Tray, Settings window, WeatherSide, Edge+Offset (no drag), Orientation, Z-order×3, Opacity, AnimationSpeed (+ pulse/breath), color palette, Sound packs, icon pack stub, **Clipboard history 1.0** (text + file paths, 1s polling, local-only, no auto-paste).
